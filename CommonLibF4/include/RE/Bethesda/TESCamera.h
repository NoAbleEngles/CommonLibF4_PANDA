...
namespace RE
{
	...
	public:
		...
		float pitch;
		float yaw;
		BSTPoint2<float> upDownInput;
		BSTPoint2<float> moveInput;
		BSTPoint2<float> rotateInput;
	};
	...
	public:
		...
		void ForceVATSMode() const
		{
			using func_t = decltype(&PlayerCamera::ForceVATSMode);
			REL::Relocation<func_t> func{ REL::ID(705260) };
			return func(this);
		}

		void StopVATSMode() const
		{
			using func_t = decltype(&PlayerCamera::StopVATSMode);
			REL::Relocation<func_t> func{ REL::ID(877415) };
			return func(this);
		}

		//Works while in VATS state or Dialogue state.
		void SetSpecialCameraPosition(const RE::NiPoint3& a_position) const
		{
			using func_t = decltype(&PlayerCamera::SetSpecialCameraPosition);
			REL::Relocation<func_t> func{ REL::ID(345417) };
			return func(this, a_position);
		}

		//Works while in VATS state or Dialogue state.
		void SetSpecialCameraRotation(float a_pitch, float a_yaw, float a_roll) const
		{
			using func_t = decltype(&PlayerCamera::SetSpecialCameraRotation);
			REL::Relocation<func_t> func{ REL::ID(79581) };
